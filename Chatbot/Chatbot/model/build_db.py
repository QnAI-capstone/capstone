...
from Chatbot.model.chunk_split import group_by_course_blocks
from Chatbot.model.flatten_notice import flat_notice, merge_by_index
import os
...
base_dir = os.path.join(os.getcwd(), "Chatbot", "model", "data")
sub_dirs = ["course", "subjectinfo", "notice"]
json_dir_list = [os.path.join(base_dir, sub_dir) for sub_dir in sub_dirs]

# ✅ 디렉토리별로 컬렉션 생성 및 파일 처리
for json_dir in json_dir_list:
    # 디렉토리 이름을 컬렉션 이름으로 사용
    dir_name = os.path.basename(json_dir)
    collection_name = f"collection_{dir_name}"
    try:
            ...
            for university_name, majors_data in base_data.items():
                ...
                for major, data in majors_data.items():
                    ...
                    if collection_name == "collection_course":
                        ...
                        chunks = group_by_course_blocks(text = json.dumps(data, ensure_ascii=False))
